...
GREEN='\033[0;32m'
NC='\033[0m'
REPO_URL="https://github.com/emreutkan/virtnet.git"
INSTALL_DIR="$HOME"
...
cd $INSTALL_DIR
echo -e "${GREEN}Cloning the virtnet repository to the root directory.${NC}"
git clone "$REPO_URL"
cd virtnet || exit
...
echo -e "${GREEN}Creating a virtnet command...${NC}"
echo "#!/bin/bash
if [ \"\$EUID\" -ne 0 ]; then
    echo 'Please run as root (sudo virtnet)'
    exit
fi
cd $INSTALL_DIR/virtnet
python virtnet.py" > virtnet

chmod +x virtnet
sudo mv virtnet /usr/local/bin/virtnet

echo -e "${GREEN}Installation complete. You can now run by typing 'sudo virtnet' in the terminal.${NC}"
